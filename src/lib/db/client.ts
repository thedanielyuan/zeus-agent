...
import { sweepTitleClaims } from "@/lib/repo/conversations";
import { startHistoryCleanup } from "@/lib/repo/maintenance";
...
export function openDb(filePath: string): Db {
  ...
  sqlite.function("zeus_contains", { deterministic: true }, (value, query) =>
    String(value).toLowerCase().includes(String(query).toLowerCase()) ? 1 : 0,
  );
...
export function getDb(): Db {
  if (!shared.zeusChatDb) {
    ...
    startHistoryCleanup(shared.zeusChatDb);
  }
